...
router.get("/", async (req, res, next) => {
  ...
  if (response) {
    ...
    console.log("from mountain project api");
    try {
      const { data } = await axios.get(
        `https://www.hikingproject.com/data/get-trails?lat=${lat}&lon=${lon}&maxResults=75&minStars=4&minLength=5&maxDistance=50&key=${key}`
      );
